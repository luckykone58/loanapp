...
<li>/</li>
<li class="text-slate-900 dark:text-white font-medium">Edit #{{ $notification->id }}</li>
@endsection
	...
	<form method="POST" action="{{ route('admin.notifications.update', $notification) }}" class="space-y-4">
		...
		<div>
			...
			<div class="relative">
				...
				<select name="user_id" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent" required>
					@foreach($users as $u)
						<option value="{{ $u->id }}" @selected(old('user_id', $notification->user_id)==$u->id)>{{ $u->username ?? $u->name }}</option>
					@endforeach
				</select>
			</div>
		...
		<div>
			...
			<div class="relative">
				...
				<input type="text" name="title" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent" value="{{ old('title', $notification->title) }}" required>
			</div>
		...
		<div>
			...
			<div class="relative">
				...
				<input type="text" name="message" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent" value="{{ old('message', $notification->message) }}" required>
			</div>
		...
		<div>
			...
			<div class="relative">
				...
				<textarea name="subtext" rows="4" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent">{{ old('subtext', $notification->subtext) }}</textarea>
			</div>
		...
		<div>
			...
			<div class="relative">
				...
				<textarea name="notes" rows="3" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent">{{ old('notes', $notification->notes) }}</textarea>
			</div>
			...
			<div class="relative">
				...
				<select name="status" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent" required>
					@foreach(['unread','read'] as $s)
						<option value="{{ $s }}" @selected(old('status', $notification->status)===$s)>{{ ucfirst($s) }}</option>
					@endforeach
				</select>
			</div>
		...
		<div>
			...
			@php
				$dt = $notification->created_date ? \Carbon\Carbon::parse($notification->created_date)->format('Y-m-d\TH:i') : '';
			@endphp
			<div class="relative">
				<span class="pointer-events-none absolute inset-y-0 left-3 flex items-center text-[#459699]">
					<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
				...
				<input type="datetime-local" name="created_date" value="{{ old('created_date', $dt) }}" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent">
			</div>
