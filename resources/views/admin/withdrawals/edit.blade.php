	...
	<form method="POST" action="{{ route('admin.withdrawals.update', $withdrawal) }}">
			...
			<div class="relative">
				...
				<select name="user_id" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent" required>
					@foreach($users as $user)
						<option value="{{ $user->id }}" @selected(old('user_id', $withdrawal->user_id)==$user->id)>{{ $user->username }}</option>
					@endforeach
				</select>
			</div>
		...
		<div class="mb-4">
			...
			<div class="relative">
				...
				<input type="number" step="0.01" name="amount" value="{{ old('amount', $withdrawal->amount) }}" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent" required>
			</div>
			...
			<div class="relative">
				...
				<select name="status" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent" required>
					@foreach(['processing','fulfilled','rejected'] as $status)
						<option value="{{ $status }}" @selected(old('status', $withdrawal->status)===$status)>{{ ucfirst($status) }}</option>
					@endforeach
				</select>
			</div>
		...
		<div class="mb-4">
			...
			<div class="relative">
				...
				<input type="text" name="withdraw_name" value="{{ old('withdraw_name', $withdrawal->withdraw_name) }}" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent">
			</div>
		...
		<div class="mb-4">
			...
			<div class="relative">
				...
				<input type="text" name="withdraw_number" value="{{ old('withdraw_number', $withdrawal->withdraw_number) }}" class="w-full border rounded pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#459699]/40 focus:border-[#459699] dark:bg-transparent">
			</div>
