@php
	$active = $active ?? null;
	$cls = fn($name) => $active === $name ? 'text-blue-600' : 'text-gray-500 hover:text-blue-600 transition-colors';
@endphp
<nav class="fixed bottom-0 left-0 right-0 max-w-[767px] mx-auto bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-50">
	<div class="flex items-center justify-around px-4 py-3 relative">
		...
		<a href="{{ route('wallets') }}" class="flex flex-col items-center gap-1 text-white cursor-pointer -mt-8">
			<div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-full p-4 shadow-lg hover:shadow-xl transition-all hover:scale-105">
				<svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
					<path d="M19 7V4a1 1 0 0 0-1-1H5a2 2 0 0 0 0 4h15a1 1 0 0 1 1 1v4h-3a2 2 0 0 0 0 4h3a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1"/>
					<path d="M3 5v14a2 2 0 0 0 2 2h15a1 1 0 0 0 1-1v-4"/>
				</svg>
			</div>
			<span class="text-xs text-blue-600 mt-1 font-bold">{{ __('footer.menu.wallet') }}</span>
		</a>
	...
	<div class="absolute inset-0 flex items-center justify-center p-4">
		<div class="w-full max-w-sm rounded-lg bg-white shadow-xl">
			<div class="px-4 py-3 border-b border-slate-200 flex items-center gap-2">
				...
				<h3 class="text-sm font-semibold text-slate-800">{{ __('footer.confirm.logout_title') }}</h3>
			</div>
			<div class="px-4 py-4 text-sm text-slate-700" id="logout-confirm-message">
			...
			<div class="px-4 py-3 border-t border-slate-200 flex justify-end gap-2">
				...
				<button type="button" id="logout-confirm-ok" class="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-red-700">
...
<script>
(function(){
	if (!window.__defaultLogoutConfirmInit) {
		window.__defaultLogoutConfirmInit = true;
